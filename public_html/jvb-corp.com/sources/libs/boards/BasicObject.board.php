...
class BasicObject {
	...
	function getLinkEdit($module,$id) {
		...
		return "<a class='edit_admin' target='_blank' href='{$bw->vars['board_url']}/admin.php?vs={$module}#{$module}/{$module}/{$module}_add_edit_form/{$id}'>Edit content</a>";
	}	
...
//		$parser->pp_nl2br = $br;
	...
	function getUrl($module = null) {
		...
		return $bw->base_url . "{$module}/detail/" . $this->getSlug(). '.html';
...
//		}
	...
	function createImageCache($fileObject, $width = 100, $height = 100, $type = 0, $noimage = 0, $class = 'class="lazy"') {
		global $bw, $vsStd;

		//$bw->input['lazy']=0;
		if(isset($bw->input['lazy']))
		{
			if($bw->input['lazy']==0 && $type==4){
				$type=3;
			}
		}
		...
		if ($type == 4) {
			//$size = $vsStd->scaleImage ( $fileObject->getPathView ( 0 ), $width, $height );
			return $this->imageCache = "<img  data-original='{$this->getCacheImagePathByFile($fileObject,$width,$height,2)}' width='$width' height='$height' src='{$bw->vars['img_url']}/lazy.jpg' {$class} alt='{$alt}' s/>";
		}
		
		if ($type == 5) {
			return $this->imageCache = "<img data-zoom-image='{$fileObject->getPathView()}' {$class} alt='{$alt}' src='{$this->getCacheImagePathByFile($fileObject,$width,$height,2)}'/>";
		}
...
EOF;
	...
	function createSeo() {
		...
		if (is_object ( $vsCom->SEO->basicObject )) {
			if (! $vsCom->SEO->basicObject->getIntro ()&&!$this->mIntro || !$this->mTitle) {
			...
			//$this->mIntro?$this->mIntro=$this->mIntro:$oIntro;
			$vsCom->SEO->basicObject->setIntro ($oIntro);
			if($this->mIntro)	
				$vsCom->SEO->basicObject->setIntro ($this->mIntro);			

			$this->mTitle?$this->mTitle=$this->mTitle:$oTitle;
			...
			if($this->mKeyword){
				$vsCom->SEO->basicObject->setKeyword ($this->mKeyword);
			}

			//pr($oTitle);die;
			
			$vsCom->SEO->basicObject->setTitle ($oTitle);
			if($this->mTitle && $this->mTitle!=""){
				$vsCom->SEO->basicObject->setTitle ($this->mTitle);
			}
