...
function createBreakCrum($obj){
		...
		$html='<div itemtype="http://data-vocabulary.org/Breadcrumb" itemscope="">'." <a class='first' href='{$bw->base_url}' itemprop='url'><span itemprop='title'>{$lang->getWords('home','Trang chủ')}</span></a>".'</div>';
		if($bw->input[0]=='abouts')
		$html.='<div itemtype="http://data-vocabulary.org/Breadcrumb" itemscope="">'." <a  href='{$obj->getUrl('abouts')}' itemprop='url'><span itemprop='title'>{$lang->getWords($bw->input[0],$bw->input[0])}</span></a>".'</div>';
		else
		...
		if(is_object($obj)){
			if(get_class($obj)=='Menu'){
				while($obj->getLevel()>1){
					...
					$array[]='<div itemtype="http://data-vocabulary.org/Breadcrumb" itemscope="">'." <a href='{$bw->base_url}{$obj->getUrl()}/category/{$obj->getSlug()}' itemprop='url'><span itemprop='title'>{$obj->getTitle()}</span></a>".'</div>';
			...
			if($cate=VSFactory::getMenus()->getCategoryById($obj->getCatId()) ){
				while($cate->getLevel()>1){
                    $array[]='<div itemtype="http://data-vocabulary.org/Breadcrumb" itemscope="">'." <a href='{$bw->base_url}{$cate->getUrl()}/category/{$cate->getSlug()}' itemprop='url'><span itemprop='title'>{$cate->getTitle()}</span></a>".'</div>';
		...
		if(is_object($obj) && $bw->input[1]!="category" ){
			//$html.='<div itemtype="http://data-vocabulary.org/Breadcrumb" itemscope="">'." <a  itemprop='url'><span itemprop='title'>{$obj->getTitle()}</span></a>".'</div>';
		}
