...
class VSFLanguage {
	...
	function getWords($key = '', $value = '') {
		...
		//pr(CACHE_PATH . "langs/" . APPLICATION_TYPE . "/".$this->getArea ());die;
		
		if (! $this->currentKeys [$module]) {
			if(!file_exists ( CACHE_PATH . "langs/" . APPLICATION_TYPE . "/". $this->getArea ()  )){
			//pr(CACHE_PATH . "langs/" . APPLICATION_TYPE . "/".$this->getArea ());die;
				mkdir(CACHE_PATH . "langs/" . APPLICATION_TYPE . "/".$this->getArea (),0750);
				chmod(CACHE_PATH . "langs/" . APPLICATION_TYPE . "/".$this->getArea (),0750);
			}
	...
	function exportToCache() {
		...
		foreach ( $this->currentKeys as $module => $value ) {
			...
			if (! is_dir ( CACHE_PATH . "langs/" . APPLICATION_TYPE . "/" . $this->getArea () )) {
				mkdir ( CACHE_PATH . "langs/" . APPLICATION_TYPE . "/" . $this->getArea (), 0750, true );
			}
