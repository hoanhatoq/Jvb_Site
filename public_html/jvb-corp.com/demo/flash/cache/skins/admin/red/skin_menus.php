...
<select name="menuCate" id="menuCate">
                    ...
                    {$this->__foreach_loop__id_53d3109b690fc($form,$message,$menu)}
                </select>
</td >
...
//===========================================================================
function __foreach_loop__id_53d3109b690fc($form=array(),$message="",$menu="")
{
global  $vsMenu,$bw;
    $BWHTML = '';
    $vsf_count = 1;
...
EOF;
if($this->getSettings()->getSystemKey($option['cate'].'_cat_seo_option',0, $option['cate'], 1, 1)) {
$BWHTML .= <<<EOF
...
<table class="obj_add_edit" width="100%"  >
 <tr>  
<td style="width:142px">{$this->getLang()->getWords('meta_title')}</td>
                       <td><input  id="menuMtTitle" style="width:99%;" type="text"  name="menuMtTitle" value="{$category->mTitle}"/></td>
</tr>
                                <tr>
<td>{$this->getLang()->getWords('meta_keywords')}</td>
                                    <td><textarea id="menuMtKeyWord" style="width:99%;" name="menuMtKeyWord">{$category->mKeyword}</textarea></td>
</tr>
                                <tr>
<td>{$this->getLang()->getWords('meta_description')}</td>
                                    <td><textarea   id="menuMtDesc" style="width:99%;" name="menuMtDesc">{$category->mIntro}</textarea></td>
</tr>
 </table></td></tr>
