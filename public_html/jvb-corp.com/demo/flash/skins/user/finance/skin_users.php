...
function loginForm_Action($option){
		...
		<form class="dangnhap" action="{$bw->base_url}users/do_login" id="login" method="POST" enctype="multipart/form-data" >
                ...
                <div class="clear_left"></div>
                <if="$option['loginUrl']"><a href="{$option['loginUrl']}"><img src="{$bw->vars['img_url']}/face_login.jpg" /></a></if>
            </form>
...
EOF;
	...
	function doChangePassword($option){
...
EOF;
	...
	function showRegList($option=array()){
...
EOF;
	...
	function showApproveList($option=array()){
...
EOF;
	...
	function viewInfo($obj,$option=array()){
  ...
  fjs.parentNode.insertBefore(js, fjs);
                    ...
                    <div class="binhluan_right">
                    	...
                    	<input type="hidden" name="comments[userId]" value="{$obj->getId()}"/>
                        	<input id="txt_poster" name="comments[poster]" vsvalue="Họ tên" type="text" style="width:303px;" onfocus="if(this.value=='Họ tên') this.value='';" onblur="if(this.value=='') this.value='Họ tên';" value="<if="$this->getUser()->basicObject->getId()">{$this->getUser()->basicObject->getTitle()}<else /> <if="$_SESSION['poster']">{$_SESSION['poster']}<else />Họ tên</if></if>" class="input_text" />
                            <br/>
...
function userComment($option=array()){
