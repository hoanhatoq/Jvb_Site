...
require_once ('./cache/skins/admin/red/skin_pages.php');
class skin_newsxx extends skin_pages {
...
$this->cate_product=VSFactory::getMenus()->getCategoryGroup('products');
...
EOF;
if($bw->input[0]=='questions') {
$BWHTML .= <<<EOF

<tr>
<td style="width: 111px;"><label>{$this->getLang()->getWords('email','Tiêu đề')}</label></td>
<td>
<input  name="{$this->modelName}[email]" type="text" value="{$obj->getEmail()}" style='width:99%' onBlur="vsf.checkPermalink($('#{$this->modelName}_title').val(),'{$bw->input[0]}')"/>
</td>
</tr>
<tr>
<td style="width: 111px;"><label>{$this->getLang()->getWords('phone','Tiêu đề')}</label></td>
<td>
<input  name="{$this->modelName}[phone]" type="text" value="{$obj->getPhone()}" style='width:99%' onBlur="vsf.checkPermalink($('#{$this->modelName}_title').val(),'{$bw->input[0]}')"/>
</td>
</tr>
...
<td>
<select  name="{$this->modelName}[catProduct]" id="vs_cate">
<option value=0 >****Chọn danh mục sản phẩm****</option>
{$this->cate_product->getChildrenBoxOption($obj->getCode())}
</select>
<br>
</td>
</tr>
