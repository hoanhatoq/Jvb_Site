...
function getTinhtrang($str=""){
	
	$hot="<div class='hot_pro'>";
...
// nd_obj_list_html#
	...
	<label>
		...
		<option <if="$bw->input['search']['status']==2">selected='selected'</if> value="2">{$this->getLang()->getWords('home')}</option>
		</select>
	</label>
	</if>
		...
		<!--<input type="hidden" value="{$obj->getSlug ()}" name="{$this->modelName}[mUrl]" id="mUrl" data-module="{$this->modelName}" data-id = "{$obj->getId()}" />-->
			<table class="obj_add_edit" width="100%">
				...
				<tr>
					...
					<td>
						<label><input <foreach="$this->hot as $key=>$value"><if="$value==1">checked="checked"</if></foreach>  name="{$this->modelName}[hot][1]"  type="checkbox" value=1>Sản phẩm mới</label>
						<label><input <foreach="$this->hot as $key=>$value"><if="$value==2">checked="checked"</if></foreach> name="{$this->modelName}[hot][2]" type="checkbox" value=2>Sản phẩm Bán chạy</label>
						<label><input <foreach="$this->hot as $key=>$value"><if="$value==3">checked="checked"</if></foreach> name="{$this->modelName}[hot][3]" type="checkbox" value=3>Sản phẩm Khuyến mãi</label>
					</td>
				</tr>
